...
private:
    ...
    static bool PatchEmbeddedPdbPath(const std::string& dllPath, const std::string& newPdbName)
    {
        ...
        {
            file.seekg(static_cast<std::streamoff>(debugOffset + i * sizeof(IMAGE_DEBUG_DIRECTORY)));
        ...
        while (!glfwWindowShouldClose(window))
        {
            ...
            /** Play-mode state machine */
            {
                ...
                if (newState != s_enginePlayState)
                {
                    ...
                    if (newState == PlayState::Editing && (s_enginePlayState == PlayState::Playing || s_enginePlayState
                        == PlayState::Paused))
                    {
